...
# Load the data.
path = 'data/'
wineQualityReds = 'wineQualityReds.csv'
df = pd.read_csv(path + wineQualityReds)
# %% markdown
...
# - Most values are 5 and 6 with a few values in other bins. The average quality is 5.6. Which is just above what would be expected at 5.5. The quality ranges from 3 to 8 which really puts us on a common 5 point scale. Overall this distribution is not very diverse.
# %% codecell
...
df.columns
plt.scatter(df['density'], df['fixed.acidity'], color='brown', alpha=0.1)
...
sns.regplot(data=df, x='density', y='fixed.acidity', color='brown')
# %% markdown
...
X = df[['density']] # independent
y = df[['fixed.acidity']] # dependent
...
plt.scatter(y_test, y_pred, color='brown', alpha=0.5)
plt.title('Model 1 testing')
plt.xlabel('Test')
plt.ylabel('Predict')
plt.savefig('figures/Test_vs_Predict.png')
# %% markdown
...
# Label it nicely
plt.title('Model 3 testing')
plt.xlabel('Test')
plt.ylabel('Predict')
plt.savefig('figures/Model3Test.png')
# %% markdown
